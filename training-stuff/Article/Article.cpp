...
void Article::setPrice(float price)
{
	if (price <= 0)
	{
		throw MyException("the price can't be zero or negative!");
	}
	_price = price;
}
...
void Article::setArticleNumber(unsigned number)
{
	if (number <= 0)
	{
		throw MyException("the article number can't be zero or negative!");
	}
...
std::string Article::getName() const
{
	if (_name.empty())
	{
		throw MyException("the name of this article is not set yet!");
	}
	return _name;
}
...
void Article::addTax(float p)
{
	if (p <= 0)
	{
		throw MyException("the value can't be zero or negative!");
	}
